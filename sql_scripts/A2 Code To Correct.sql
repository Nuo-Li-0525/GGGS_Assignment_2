DECLARE
  ...
  CURSOR c_gggs IS
    SELECT *
      ...
      WHERE data_processed = k_data_unprocessed --> Logic Error 4: Missing 'WHERE' clause
    ORDER BY loadID;  
  ...
  FOR r_gggs IN c_gggs LOOP
    BEGIN 
        ...
        ELSIF (r_gggs.process_type = k_change) THEN
          ...
          UPDATE gggs_customer
             SET province = DECODE(r_gggs.column2, k_no_change_char, province, r_gggs.column2),
                 ...
                 phone_number = NVL2(r_gggs.column6,r_gggs.column6, phone_number) --> Syntax Error 1: NVL -> NVL2
           WHERE name = r_gggs.column1;  
      ...
      ELSIF (r_gggs.data_type = k_vendor) THEN
        ...
        IF (r_gggs.process_type = k_new) THEN
          ...
          VALUES (gggs_vendor_seq.NEXTVAL, r_gggs.column1, r_gggs.column2, r_gggs.column3,
                  r_gggs.column4, r_gggs.column6, k_active_status); --> Logic Error 1: k_status -> k_active_status

        ELSIF (r_gggs.process_type = k_status) THEN --> Syntax Error 2: k_stats -> k_status
          UPDATE gggs_vendor
             SET status = r_gggs.column2
           WHERE name = r_gggs.column1;    
        ELSIF (r_gggs.process_type = k_change) THEN
          ...
          UPDATE gggs_vendor
           ...
           WHERE name = r_gggs.column1; --> Syntax Error 3: missing ';' to end statement
      ...
      ELSIF (r_gggs.data_type = k_stock) THEN
        ...
        IF (r_gggs.process_type = k_new) THEN
          SELECT categoryID INTO v_name1
            FROM gggs_category
           WHERE name = r_gggs.column1;

          SELECT vendorID INTO v_name2
            FROM gggs_vendor
           WHERE name = r_gggs.column2; --> Logic Error 2: r_gggs.column3 -> r_ggs.column2

          INSERT INTO gggs_stock 
          VALUES (gggs_stock_seq.NEXTVAL, v_name1, v_name2, r_gggs.column1, --> Logic Error 5: r_gggs.column3 -> r_gggs.column1
        ...
        ELSIF (r_gggs.process_type = k_change) THEN --> Syntax Error 4: 'ELSE IF' not valid in PL/SQL: ELSE IF -> ELSIF
          UPDATE gggs_stock
            SET description = DECODE(r_gggs.column4, k_no_change_char, description, r_gggs.column4),
                 ...
                 no_in_stock = NVL2(r_gggs.column8, (no_in_stock + r_gggs.column8), no_in_stock) --> Logic Error 3: '-' -> '+'
           WHERE name = r_gggs.column1;
    ...
    EXCEPTION 
      WHEN OTHERS THEN
        ...
        VALUES (r_gggs.data_type, r_gggs.process_type, v_message);
        COMMIT;
    END; --> Syntax Error 5: Missing 'END;' statement
  END LOOP;
END;
/